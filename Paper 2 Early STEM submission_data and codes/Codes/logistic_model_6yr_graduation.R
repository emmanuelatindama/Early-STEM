...
set.seed(15)
train_index <- sample(1:nrow(logistic6yrdata), size = 0.8*nrow(logistic6yrdata))
...
predict_6yr_reg <- as.factor(predict_6yr_reg)
expected_values <- as.factor(testlogistic6yr$Graduated_within_6_years)

#==============================Evaluating model accuracy using confusion matrix (Accuracy, Sensitivity and Specificity =================================================================
library(caret)
results <- confusionMatrix(data=predict_6yr_reg, reference = expected_values)
results
#================================Creating ROC curve====================================================================================================
library(pROC)
test_prob = predict(logistic_6yr_model, newdata = testlogistic6yr, type = "response")
test_roc = roc(testlogistic6yr$Graduated_within_6_years ~ test_prob, plot = TRUE, print.auc = TRUE)
